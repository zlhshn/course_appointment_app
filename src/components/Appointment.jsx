    ...
    <div className="p-4">
      ...
      <Container>
        <Row className="text-left  p-3">
          {data.length < 1 && (
            <img src="./img/appointment.jpg" className="app-img" />
          )}

          {data.map(({ student, day, teacher, id, booked, time }) => {
            return (
              <div
                onDoubleClick={() => handleBooked(id)}
                key={id}
                className="d-flex justify-content-between app-container g-3 rounded-2  align-items-center p-3 "
              >
                <div>
                  <h5>
                    <span className="text-danger">Instructor:</span> {teacher}
                  </h5>
                  <h5>
                    <span className="text-danger">Student:</span> {student}
                  </h5>
                </div>
                {booked ? (
                  <Booked />
